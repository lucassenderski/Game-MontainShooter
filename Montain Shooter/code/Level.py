...
class Level:
    ...
    def run(self, player_score: list[int]):
        ...
        while True:
            ...
            # Desenha e movimenta todas as entidades
            for ent in self.entity_list:
                self.window.blit(source=ent.surf, dest=ent.rect)
                ent.move()

                # Lógica de tiro para jogadores e inimigos
            ...
            for event in pygame.event.get():
                ...
                if event.type == EVENT_ENEMY:
                    if self.name == 'Level1':
                        choice = random.choice(('Enemy1', 'Enemy1', 'Enemy2'))  # Mais Enemy1 no level 1
                    elif self.name == 'Level2':
                        choice = random.choice(('Enemy1', 'Enemy2', 'Enemy2'))  # Mais Enemy2 no level 2
                    elif self.name == 'Level3':
                        choice = random.choice(('Enemy1', 'Enemy2', 'Enemy3'))  # Inclui Enemy3 no level 3
                    self.entity_list.append(EntityFactory.get_entity(choice))

                # Contagem regressiva do level
                if event.type == EVENT_TIMEOUT:
                    self.timeout -= TIMEOUT_STEP
                    if self.timeout == 0:
                        # Salva os scores dos jogadores
                        for ent in self.entity_list:
                            if isinstance(ent, Player) and ent.name == 'Player1':
                                player_score[0] = ent.score
                            if isinstance(ent, Player) and ent.name == 'Player2':
                                player_score[1] = ent.score
                        return True  # Level completado com sucesso
            ...
            if not found_player:
                # Salva os scores dos jogadores mesmo em caso de game over
                for ent in self.entity_list:
                    if isinstance(ent, Player) and ent.name == 'Player1':
                        player_score[0] = ent.score
                    if isinstance(ent, Player) and ent.name == 'Player2':
